...
import {
    ...
    ProductListProps,
    ProductProps,
    REMOVE_PRODUCT_FROM_PRODUCT_DETAIL,
...
export const addProductFromProductDetail = createAsyncThunk<AddProductResponsProps, AddProductProps, { rejectValue: string }>(
    ADD_PRODUCT_FROM_PRODUCT_DETAIL,
    async (data, { rejectWithValue }) => {
        try {
            const result = await addProduct(data);
...
export const removeProductFromProductDetail = createAsyncThunk<AddProductResponsProps, RemoveProductProps, { rejectValue: string }>(
    REMOVE_PRODUCT_FROM_PRODUCT_DETAIL,
    async (data, { rejectWithValue }) => {
        try {
            const items = await removerProduct(data);
